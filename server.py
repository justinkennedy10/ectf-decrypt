...
def main():
    ...
    data_queue = bytearray(data)

    if struct.unpack("I", bytes(data_queue[0:4]))[0] == 0:
        message_length = struct.unpack("I", bytes(data_queue[4:8]))[0]

    request = json_loads_byteified(private_key.decrypt(bytes(data_queue[8:message_length+8])))
    print request
    data_queue = data_queue[message_length+8:]
